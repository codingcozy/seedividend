...
it("상품의 수량을 증가시켜야 합니다", () => {
  ...
  const total: HTMLDivElement = fixture.debugElement.query(By.css('[data-testid="total"]')).nativeElement;
  expect(total.textContent).toBe("18");

  fixture.debugElement.query(By.css('[data-testid="btn-increase"]')).nativeElement.click();
  fixture.detectChanges();
...
export class BasketService {
  ...
  totalPrice = computed(() => this.products().reduce((total, product) => total + product.price * product.amount, 0));

  decrease(id: number) {
    this.#change(id, (product) => (product.amount > 0 ? { ...product, amount: product.amount - 1 } : product));
  }
