...
<img src="/assets/img/BuildingaTreeTableUsingAntDesignSystem_4.png" />
...
const getTableRowCount = (data = []) =>
  data.reduce((count, item) => (item.children ? count + getTableRowCount(item.children) + 1 : count + 1), 0);
```
...
const getAllParentNodeKeys = (data = []) =>
  data.reduce((list, item) => (item.children ? [...list, item.key, ...getAllParentNodeKeys(item.children)] : list), []);
```
