...
사용자별 연관된 기사에 액세스하면서 게으른 초기화를 트리거하는 경우, 총 몇 개의 쿼리가 실행될까요? 7 + 1이 될까요? 5 + 1이 될까요? 아니요, 정답은 여전히 4입니다. Hibernate가 사용자별 기사에 대한 게으른 초기화를 트리거할 때, 카테고리를 Left Join Fetch도 함께 가져옵니다. 각 게으른 쿼리는 다음과 같이 보이는 SQL 문을 실행합니다: select a._, c._ from article a left join category c on c.id = a.category_id where a.author = ?
