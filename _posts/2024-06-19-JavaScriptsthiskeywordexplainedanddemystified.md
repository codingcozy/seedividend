...
console.log(person2.age); // // => TypeError: Cannot read property 'age' of undefined
```
...
const person2 = Person();
console.log(person2.age); // // => Must use the new operator to call the function
```
...
getContinent.call(nigeria, "나이지리아는");
getContinent.call(china, "중국은");
```
...
```js
getContinent.apply(nigeria, ["나이지리아는"]);
getContinent.apply(china, ["중국은"]);
```
...
console.log(boundGetX()); // 기대 출력: 42
```
