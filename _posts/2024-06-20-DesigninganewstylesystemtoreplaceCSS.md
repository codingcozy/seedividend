...
```js
Text("Hello World").fontFamily("Helvetica").fontColor("black").opacity(0.5);
```
...
```js
Text("안녕하세요 세계").fontFamily("Helvetica").fontFamily(); // 현재 글꼴 가져오기
```
...
Stack(
  Text("Hello World").fontColor(color),

  Text("Hola mundo").fontColor(color)
);
```
...
```js
const catImage = Image("cat.jpeg").width(100).height(300);

const dogImage = Image("dog.jpeg").width(catImage.width()).height(catImage.height());
```
...
Stack(Button("Log in"), Button("Subscribe"));
```
